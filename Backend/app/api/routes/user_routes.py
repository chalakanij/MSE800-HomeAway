...
async def register(user: EmployerCreate, db: Session = Depends(get_db)):
...
async def deactivate_users_list(users: UserDeactivateRequest, db:Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    user_service = UserService(db)
...
async def read_profile(db: Session = Depends(get_db), current_user: EmployeeOutput = Depends(get_current_user)):
    return current_user

@router.put("/profile", response_model=EmployeeOutput, description="Update current user's profile")
async def update_profile_request(user_profile: ProfileInput ,db: Session = Depends(get_db), current_user: EmployeeOutput = Depends(get_current_user)):
    user_service = UserService(db)
